...
public enum PreEventType {
    case join(state: String, email: EmailJoinView, apple: AppleView, google: GoogleOAuthView)
    case resetPassword(input: PasswordResetView)
    case setPassword(input: PasswordSetView)
    case signIn(state: String, email: EmailSignInView, apple: AppleView, google: GoogleOAuthView)
}
